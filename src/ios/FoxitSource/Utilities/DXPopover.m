...
{
    return [[[DXPopover alloc] init] autorelease];
}
...
- (void)dealloc
{
    [_didShowHandler release];
    [_didDismissHandler release];
    [_blackOverlay release];
...
- (void)showAtPoint:(CGPoint)point popoverPosition:(DXPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView
{
    ...
    if (!self.blackOverlay) {
        self.blackOverlay = [[[UIControl alloc] init] autorelease];
...
{
    UIBezierPath *arrow = [[[UIBezierPath alloc] init] autorelease];
