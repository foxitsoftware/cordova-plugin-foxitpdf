...
{
	if (self = [super init]) {
        _pdfViewCtrl = pdfReader.pdfViewCtrl;
...
- (void)gotoPageWithPageIndex:(int)pageIndex
{
	if (pageIndex <= 0 || pageIndex > [_pdfViewCtrl getPageCount]) {
		...
		AlertView *alertView = [[AlertView alloc]
								 ...
								 message:[NSString stringWithFormat:@"%@ %d - %d", NSLocalizedStringFromTable(@"kWrongPageNumber", @"FoxitLocalizable", nil), 1, [_pdfViewCtrl getPageCount]]
...
- (void)quitGotoMode
{
    ...
    [_pdfReader changeState:_oldReadFrameState];
}
...
- (void)showGotoPageToolbar:(UITapGestureRecognizer *)recognizer
{
    ...
    if([_pdfReader getState] != STATE_PAGENAVIGATE)
        _oldReadFrameState = [_pdfReader getState];
    [_pdfReader changeState:STATE_PAGENAVIGATE];
}
...
-(void)loadModule
{
	...
	[_pdfReader registerFullScreenListener:self];
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation{
    ...
    [self setPageCountLabel];
    int state = [_pdfReader getState];
    if (state == STATE_THUMBNAIL || state == STATE_ANNOTTOOL || state == STATE_EDIT || state == STATE_SIGNATURE || _pdfReader.isFullScreen) {
        self.pageNumView.hidden = YES;
    }else
    {
       self.pageNumView.hidden = NO;
