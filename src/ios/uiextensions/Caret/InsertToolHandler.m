...
- (BOOL)onPageViewTap:(int)pageIndex recognizer:(UITapGestureRecognizer *)recognizer
{
    ...
    [NoteDialog setViewCtrl:self.pdfViewCtrl];
    [NoteDialog defaultNoteDialog].title = NSLocalizedStringFromTable(@"kInsertText", @"FoxitLocalizable", nil);//todel
    ...
    [NoteDialog defaultNoteDialog].noteEditDone = ^()
    {
        ...
        annot.color = [self.extensionsManager getPropertyBarSettingColor:self.type];
        annot.opacity = [self.extensionsManager getAnnotOpacity:self.type] / 100.0f;
        ...
        if (annot) {
            [[self.extensionsManager getAnnotHandlerByAnnot:annot] addAnnot:annot addUndo:YES];
        }
        [self clearSelection];
...
{
    if (self != [self.extensionsManager getCurrentToolHandler]) {
        return NO;
    }
    return YES;
}
