...
-(void)loadModule
{
    ...
    [_pdfReader registerStateChangeListener:self];
    [_pdfViewCtrl registerDocEventListener:self];
    [_pdfViewCtrl registerPageEventListener:self];
    FSPDFReader* __weak weakReadFrame = _pdfReader;
    _pdfReader.settingBarController.settingBar.onlytext = ^(BOOL selected) {
...
- (void)enterReflowMode:(BOOL)flag
{
	if (flag) {
		...
		[_pdfReader changeState:STATE_REFLOW];
		...
		[_pdfReader changeState:STATE_NORMAL];
        [_pdfReader.settingBarController.settingBar setItemState:NO value:0 itemType:ONLYTEXT];
...
- (UIView *)viewTopReflow
{
	if (!_viewTopReflow) {
		...
		self.titleItem = [TbBaseItem createItemWithTitle:NSLocalizedStringFromTable(@"kReflow", @"FoxitLocalizable", nil)]; // assign to var to keep a strong reference, can't directly assign to weak property of self
...
- (UIToolbar *)toolBarBottomReflow
{
	if (!_toolBarBottomReflow) {
		...
		if(!(_extensionsManager.modulesConfig.loadReadingBookmark || _extensionsManager.modulesConfig.loadOutline || _extensionsManager.modulesConfig.loadAnnotations || _extensionsManager.modulesConfig.loadAttachment))
...
{
	if ([_pdfReader getState] == STATE_REFLOW) {
		if (self.viewTopReflow.hidden) {
			[self showReflowBar:YES animation:YES];
			_pdfReader.isFullScreen = NO;
            [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:YES];
		} else {
			[self showReflowBar:NO animation:YES];
			_pdfReader.isFullScreen = YES;
