...
- (void)onAnnotSelected:(FSAnnot *)annot {
    ...
    MenuItem *editItem = [[MenuItem alloc] initWithTitle:FSLocalizedString(@"kEdit") object:self action:@selector(edit)];
...
- (void)fixInvalidFontForFreeText:(FSFreeText *)annot {
    ...
    if (!ap.font || ap.fontSize == 0) {
        if (!ap.font) {
            ap.font = [[FSFont alloc] initWithStandardFontID:e_fontStandardIDHelvetica];
        }
        if (ap.fontSize == 0) {
            ap.fontSize = 10;
        }
...
- (void)keyboardWasShown:(NSNotification *)aNotification {
    ...
    if (annot) {
        ...
        if (CGRectGetHeight(_pdfViewCtrl.bounds) - dvAnnotRect.origin.y - dvAnnotRect.size.height < keyboardFrame.size.height) {
            float dvOffsetY = keyboardFrame.size.height - (CGRectGetHeight(_pdfViewCtrl.bounds) - dvAnnotRect.origin.y - dvAnnotRect.size.height) + 60;
