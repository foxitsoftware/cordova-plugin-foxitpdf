...
- (void)viewDidLoad {
    ...
    [self loadCertInfo];
    UIBarButtonItem *leftButton = [[UIBarButtonItem alloc] initWithTitle:FSLocalizedString(@"kCancel") style:UIBarButtonItemStylePlain target:self action:@selector(didCancel)];
    self.navigationItem.leftBarButtonItem = leftButton;
    UIBarButtonItem *rightButton = [[UIBarButtonItem alloc] initWithTitle:FSLocalizedString(@"kOK") style:UIBarButtonItemStylePlain target:self action:@selector(didOk)];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.dataArray.count < 1) {
        ...
        nonCertLabel.text = FSLocalizedString(@"kNoCert");
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if ([self.currentCertPasswordDic objectForKey:md5]) {
        ...
        certInfoCtr.title = FSLocalizedString(@"kCertInfo");
...
- (void)promptForPassword:(void (^)(NSString *))complete {
    ...
    if (self.showPromptTimes == 0) {
        alertView.title = FSLocalizedString(@"kOfflineCopyAlterTitle");
    } else {
        alertView.title = FSLocalizedString(@"kPassWordErrorAlterTitle");
    }
