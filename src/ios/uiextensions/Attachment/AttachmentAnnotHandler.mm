    ...
    if ([Utility isSupportFormat:attachmentPath]) {
        ...
        [rootViewController presentViewController:attachmentCtr animated:YES completion:^{
            ...
            dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [attachmentCtr openDocument:attachmentPath];
                });
            });
...
- (BOOL)onPageViewPan:(int)pageIndex recognizer:(UIPanGestureRecognizer *)recognizer annot:(FSAnnot*)annot
{
    ...
    else if (recognizer.state == UIGestureRecognizerStateChanged)
    {
        ...
        if (!(newRect.origin.x <= 0
              || newRect.origin.x + newRect.size.width >= pageView.bounds.size.width
              || newRect.origin.y <= 0
              || newRect.origin.y + newRect.size.height >= pageView.bounds.size.height)) {
