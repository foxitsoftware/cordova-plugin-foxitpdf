    ...
    selectDestination.operatingHandler = ^(FileSelectDestinationViewController *controller, NSArray *destinationFolder)
    {
        ...
        if (destinationFolder.count > 0)
        {
            ...
            if (fileSize > 50*1024*1024) { // 50MB
                AlertView *alertView = [[AlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"kWarning", @"FoxitLocalizable", nil) message:[NSString stringWithFormat:NSLocalizedStringFromTable(@"kAttachmentMaxSize", @"FoxitLocalizable", nil) ,50] buttonClickHandler:nil cancelButtonTitle:@"kOK" otherButtonTitles:nil];
                [alertView show];
                return;
            }
