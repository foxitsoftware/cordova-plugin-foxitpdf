...
-(instancetype)initWithPDFViewCtrl:(FSPDFViewCtrl*)pdfViewCtrl pdfReader:(FSPDFReader*)pdfReader
{
    ...
    if (self) {
        ...
        self.settingBar = [[SettingBar alloc] initWithPDFViewCtrl:pdfViewCtrl moreSettingBarController:self pdfReader:self.pdfReader];
...
{
    if (DEVICE_iPHONE) {
        self.settingBar.singleView_iphone.selected = (newLayoutMode == PDF_LAYOUT_MODE_SINGLE);
    }
    else
    {
        self.settingBar.singleView_ipad.selected = (newLayoutMode == PDF_LAYOUT_MODE_SINGLE);
    }
    
    if (DEVICE_iPHONE) {
        self.settingBar.continueView_iphone.selected = (newLayoutMode == PDF_LAYOUT_MODE_CONTINUOUS);
    }
    else
    {
        self.settingBar.continueView_ipad.selected = (newLayoutMode == PDF_LAYOUT_MODE_CONTINUOUS);
    }
    
    self.settingBar.doubleView_ipad.selected = (newLayoutMode == PDF_LAYOUT_MODE_TWO);
    
    if (DEVICE_iPHONE) {
        self.settingBar.thumbnailView_iphone.selected = (newLayoutMode == PDF_LAYOUT_MODE_MULTIPLE);
    }
    else
    {
        self.settingBar.thumbnailView_ipad.selected = (newLayoutMode == PDF_LAYOUT_MODE_MULTIPLE);
    }
}
