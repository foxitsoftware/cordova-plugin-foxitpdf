...
- (instancetype)initWithReadFrame:(FSPDFReader*)pdfReader
{
	...
	if (self)
    {
        ...
        fileBrowser.top = NO;
        TbBaseItem* documentTitle = [TbBaseItem createItemWithTitle:NSLocalizedStringFromTable(@"kDocuments", @"FoxitLocalizable", nil)];
        ...
        if (DEVICE_iPHONE)
        {
            ...
            if (sortType == FileSortType_Name) {
                if (sortMode == FileSortMode_Ascending) {
                    ...
                    sortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kName", @"FoxitLocalizable", nil) imageNormal:imageNormal imageSelected:imageSelected imageDisable:nil background:nil imageTextRelation:RELATION_LEFT];
                }
                else
                {
            ...
            else if(sortType == FileSortType_Date)
            {
                if (sortMode == FileSortMode_Ascending) {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    UIImage *imageSelected = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    sortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kTimeSort", @"FoxitLocalizable", nil) imageNormal:imageNormal imageSelected:imageSelected imageDisable:nil background:nil imageTextRelation:RELATION_LEFT];
                }
                else
                {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortselect_black.png"];
            ...
            else if(sortType == FileSortType_Size)
            {
                if (sortMode == FileSortMode_Ascending) {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    UIImage *imageSelected = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    sortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kSize", @"FoxitLocalizable", nil) imageNormal:imageNormal imageSelected:imageSelected imageDisable:nil background:nil imageTextRelation:RELATION_LEFT];
                }
                else
                {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortselect_black.png"];
        ...
        else
        {
            ...
            self.ipadsortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kName", @"FoxitLocalizable", nil) imageNormal:[UIImage imageNamed:@"document_sortblank"] imageSelected:[UIImage imageNamed:@"document_sortupselect"] imageDisable:[UIImage imageNamed:@"document_sortblank"] background:nil imageTextRelation:RELATION_LEFT];
            ...
            self.ipadsortTimeItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kTimeSort", @"FoxitLocalizable", nil) imageNormal:[UIImage imageNamed:@"document_sortblank"] imageSelected:[UIImage imageNamed:@"document_sortupselect"] imageDisable:[UIImage imageNamed:@"document_sortblank"] background:nil imageTextRelation:RELATION_LEFT];
            ...
            self.ipadsortTypeItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kSize", @"FoxitLocalizable", nil) imageNormal:[UIImage imageNamed:@"document_sortblank"] imageSelected:[UIImage imageNamed:@"document_sortupselect.png"] imageDisable:[UIImage imageNamed:@"document_sortblank"] background:nil imageTextRelation:RELATION_LEFT];
...
- (UIView *) configureSortPopoverView
{
    ...
    date.tag = 200;
    [date setTitle:NSLocalizedStringFromTable(@"kTimeSort", @"FoxitLocalizable", nil) forState:UIControlStateNormal];
    ...
    name.tag = 210;
    [name setTitle:NSLocalizedStringFromTable(@"kName", @"FoxitLocalizable", nil) forState:UIControlStateNormal];
    ...
    size.tag = 220;
    [size setTitle:NSLocalizedStringFromTable(@"kSize", @"FoxitLocalizable", nil) forState:UIControlStateNormal];
...
- (void) setSortButtonPopover
{
    if (DEVICE_iPHONE)
    {
        ...
        UIViewController * vc = [self.pdfReader.rootViewController.viewControllers objectAtIndex:0];
...
- (void)fileSort:(id)sender
{
    ...
    if (button.tag == 200) // sort by time
    {
        ...
        self.sortNameItem.text = NSLocalizedStringFromTable(@"kTimeSort", @"FoxitLocalizable", nil);
        if (button.selected) {
    ...
    else if (button.tag == 210) // sort by name
    {
        ...
        self.sortNameItem.text = NSLocalizedStringFromTable(@"kName", @"FoxitLocalizable", nil);
    ...
    else if (button.tag == 220) // sort by size
    {
        ...
        self.sortNameItem.text = NSLocalizedStringFromTable(@"kSize", @"FoxitLocalizable", nil);
...
- (void)onItemCliked:(FbFileItem *)item
{
    if (!item.isFolder)
    {
        [self.pdfReader openPDFAtPath:item.path withPassword:nil];
        item.isOpen = YES;
    }
    else
    {
