...
-(instancetype)initWithExtensionsManager:(UIExtensionsManager*)extensionsManager pdfReader:(FSPDFReader*)pdfReader
{
	if (self = [super init]) {
        ...
        {
            MenuGroup* group = [_pdfReader.more getGroup:TAG_GROUP_PROTECT];
            if (!group) {
                group = [[MenuGroup alloc] init];
                group.tag = TAG_GROUP_PROTECT;
                
                group.title = NSLocalizedStringFromTable(@"kSecurity", @"FoxitLocalizable", nil);
                [_pdfReader.more addGroup:group];
            }
            MvMenuItem *pwdItem = [[MvMenuItem alloc] init];
            pwdItem.tag = TAG_ITEM_PASSWORD;
            pwdItem.text = NSLocalizedStringFromTable(@"kEncryption", @"FoxitLocalizable", nil);
            pwdItem.callBack = self;
            [_pdfReader.more addMenuItem:TAG_GROUP_PROTECT withItem:pwdItem];
        }
...
- (void)tryLoadPDFDocument:(FSPDFDoc*)document guessPassword:(NSString*)guessPassword success:(void(^)(NSString* password))success error:(void(^)(NSString* description))error abort:(void(^)())abort
{
    ...
    if(status == e_errSuccess) {
        ...
        NSString* title = guessPassword ? NSLocalizedStringFromTable(@"kDocPasswordError", @"FoxitLocalizable", nil) : NSLocalizedStringFromTable(@"kDocNeedPassword", @"FoxitLocalizable", nil);
...
- (void)onClick:(MvMenuItem *)item
{
    ...
    if ([item.text isEqualToString:NSLocalizedStringFromTable(@"kEncryption", @"FoxitLocalizable", nil)]) {
...
{
    NSString* originalPDFPath = _pdfReader.filePath;
...
- (BOOL)removeEncrypt:(FSPDFDoc*)pdfDoc
{
    ...
    @autoreleasepool {
        ...
        if (isOK) {
            NSString* fileName = [_pdfReader.filePath lastPathComponent];
        ...
        if (isOK) {
            isOK = [fileManager removeItemAtPath:_pdfReader.filePath error:nil];
            
            if (isOK) {
                isOK = [fileManager moveItemAtPath:decryptedPDFPath toPath:_pdfReader.filePath error:nil];
            }
        }
        if (isOK) {
            FSPDFDoc* pdfDoc = [FSPDFDoc createFromFilePath:_pdfReader.filePath];
...
- (void)onDocOpened:(FSPDFDoc*)document error:(int)error
{
    ...
    NSString* text = NSLocalizedStringFromTable(@"kEncryption", @"FoxitLocalizable", nil);
    if (self.passwordType == e_pwdUser ||
        self.passwordType == e_pwdOwner) {
        text = NSLocalizedStringFromTable(@"kDecryption", @"FoxitLocalizable", nil);
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!isOK) {
                ...
                [self updatePasswordType];
                [self updateItemText:NSLocalizedStringFromTable(@"kDecryption", @"FoxitLocalizable", nil)];
...
- (void)handleRemoveEncryption
{
    ...
    __block void (^promptForOwnerPassword) (NSString* title, SuccessCallback success) = ^(NSString* title, SuccessCallback success) {
        [weakSelf promptForPasswordWithTitle:title callback:^(BOOL isInputed, NSString* password) {
            if (isInputed) {
                ...
                if (!isOwner) {
                    promptForOwnerPassword(NSLocalizedStringFromTable(@"kDocPasswordError", @"FoxitLocalizable", nil), success);
                } else if (success) {
    ...
    void (^tryRemoveEncryption)() = ^{
        if ([weakSelf isOwner]) {
            ...
            promptForOwnerPassword(NSLocalizedStringFromTable(@"kDocNeedPassword", @"FoxitLocalizable", nil), ^{
                [weakSelf removeEncryption];
            });
        }
    };
    if ([self.pdfDoc isModified]) { // whether to save before remove encryption?
        AlertView *alertView = [[AlertView alloc] initWithTitle:@"kConfirm" message:@"Do you want to save document before removing encryption?" buttonClickHandler:^(UIView *alertView, int buttonIndex) {
            ...
            if (!shouldSaveChanges) {
                FSPDFDoc* pdfDoc = [FSPDFDoc createFromFilePath:_pdfReader.filePath];
                enum FS_ERRORCODE error = [pdfDoc load:_pdfReader.password];
                if (error == e_errSuccess) {
                    weakSelf.pdfDoc = pdfDoc;
                } else {
                    //todo
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!isOK) {
                ...
                [self updateItemText: NSLocalizedStringFromTable(@"kEncryption", @"FoxitLocalizable", nil)];
...
{
    MenuGroup * securityGroup = [_pdfReader.more getGroup:TAG_GROUP_PROTECT];
