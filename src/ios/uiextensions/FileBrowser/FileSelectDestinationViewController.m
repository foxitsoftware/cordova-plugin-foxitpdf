...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        ...
        self.fileTypeImage.layer.borderColor  = [UIColor colorWithHexString:@"D8D8D8"].CGColor;
...
- (void)viewDidLoad
{
    ...
    if (self.fileOperatingMode == FileListMode_Copy || self.fileOperatingMode == FileListMode_Move || self.fileOperatingMode == FileListMode_Select || self.fileOperatingMode == FileListMode_SaveTo)
...
- (void) configureInstructionMessage
{
    ...
    CGSize titleSize = [Utility getTextSize:NSLocalizedStringFromTable(msg, @"FoxitLocalizable", nil) fontSize:20.0f maxSize:CGSizeMake(SCREENWIDTH - 60, 100)];
    ...
    if (lblInstructionMsg == nil)
    {
        ...
        lblInstructionMsg.text = NSLocalizedStringFromTable(msg, @"FoxitLocalizable", nil);
...
- (void)initNavigationBar
{
    ...
    [buttonLeft setTitle:NSLocalizedStringFromTable(@"kCancel", @"FoxitLocalizable", nil) forState:UIControlStateNormal];
    ...
    {
       [self.buttonDone setTitle:NSLocalizedStringFromTable(@"kCopy", @"FoxitLocalizable", nil)];
       self.title = NSLocalizedStringFromTable(@"kCopyToFolder", @"FoxitLocalizable", nil);
    }
    else if (self.fileOperatingMode == FileListMode_Move)
    {
       self.title = NSLocalizedStringFromTable(@"kMoveToFolder", @"FoxitLocalizable", nil);
       [self.buttonDone setTitle:NSLocalizedStringFromTable(@"kMove", @"FoxitLocalizable", nil)];
    ...
    {
       self.title = NSLocalizedStringFromTable(@"kCloudUploadFile", @"FoxitLocalizable", nil);
       [self.buttonDone setTitle:NSLocalizedStringFromTable(@"kCloudUploadFile", @"FoxitLocalizable", nil)];
    }
    else if (self.fileOperatingMode == FileListMode_Select)
    {
        [self.buttonDone setTitle:NSLocalizedStringFromTable(@"kOK", @"FoxitLocalizable", nil)];
    }
    else if (self.fileOperatingMode == FileListMode_Import)
    {
        [self.buttonDone setTitle:NSLocalizedStringFromTable(@"kSelectFile", @"FoxitLocalizable", nil)];
        [self.buttonDone setEnabled:NO];
    }
    else if (self.fileOperatingMode == FileListMode_SaveTo)
    {
        [self.buttonDone setTitle:NSLocalizedStringFromTable(@"kSave", @"FoxitLocalizable", nil)];
        self.title = NSLocalizedStringFromTable(@"kSelectToFolder", @"FoxitLocalizable", nil);
    }
...
- (void)loadFilesWithPath:(NSString *)filePath
{
    ...
    {
        [self.fileItemsArray addObject:NSLocalizedStringFromTable(@"kBack", @"FoxitLocalizable", nil)];
    }
...
- (FileViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                folderCell.fileTypeImage.image = [Utility drawPageThumbnailWithPDFPath:fileItem.filekey pageIndex:0 pageSize:CGSizeMake(41, 58)];
            }
            else
            {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!fileItem.isFolder)
    {
        ...
        if (self.fileOperatingMode == FileListMode_Upload) {
            ...
            {
                self.title = [NSString stringWithFormat:@"%@(%lu)",NSLocalizedStringFromTable(@"kCloudUploadFile", @"FoxitLocalizable", nil),(unsigned long)self.selectedArray.count];
            }
        }
        else
