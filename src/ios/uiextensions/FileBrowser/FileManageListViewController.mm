...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.grayView.tag == 100)
    {
        if (indexPath.row == 0)
        {
            ...
            cell.textLabel.text = NSLocalizedStringFromTable(@"kZip", @"FoxitLocalizable", nil);
            
        }
        else if (indexPath.row == 1)
        {
            imageView.image = [UIImage imageNamed:@"document_edit_share.png"];
            cell.textLabel.text = NSLocalizedStringFromTable(@"kShare", @"FoxitLocalizable", nil);
        }
    }
    else
    {
...
- (ThumbnailScrollViewCell *)thumbnailScrollView:(ThumbnailScrollView *)scrollView cellAtIndex:(NSInteger)index
{
    ...
    {
        imageViewThumbnail.layer.borderWidth  = 0.0;
        labelNumberTitle.text = [NSString stringWithFormat:NSLocalizedStringFromTable(@"kItems", @"FoxitLocalizable", nil), fileObj.fileSize];
...

{
    ...
    if ([[FSPDFReader sharedInstance].folderSizeDictionary valueForKey:key]) {
        return [[[FSPDFReader sharedInstance].folderSizeDictionary valueForKey:key] unsignedLongLongValue];
    }
...
//            if ([fileName.pathExtension isEqualToString:@"pdf"])
    ...
    [[FSPDFReader sharedInstance].folderSizeDictionary setValue:[NSNumber numberWithUnsignedLongLong:folderSize] forKey:key];
    return folderSize;
}
