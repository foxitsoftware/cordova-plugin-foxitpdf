...
{
    _searchBar.placeholder = NSLocalizedStringFromTable(@"kSearchPlaceholder", @"FoxitLocalizable", nil);
}
...
- (BOOL)checkDiskFile:(int)currentSelectRow
{
    ...
    {
        AlertView *alertView = [[AlertView alloc] initWithTitle:@"kWarning" message:[NSString stringWithFormat:NSLocalizedStringFromTable(@"kNoFileInDisk", @"FoxitLocalizable", nil), selectedFile.fileName] buttonClickHandler:nil cancelButtonTitle:nil otherButtonTitles:@"kOK", nil];
        [alertView show];
...
- (void)setNavigationBarTitleForFolderName
{
    ...
    {
        self.title = NSLocalizedStringFromTable(@"kDocuments", @"FoxitLocalizable", nil);
    }
    else
    {
        {
