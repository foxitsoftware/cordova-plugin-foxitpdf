...
+ (NSMutableDictionary<NSString *, NSNumber *> *)getFolderSizeDictionary {
    static NSMutableDictionary<NSString *, NSNumber *> *folderSizeDictionary = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        folderSizeDictionary = [NSMutableDictionary<NSString *, NSNumber *> dictionary];
    });
    return folderSizeDictionary;
}
...
- (ThumbnailScrollViewCell *)thumbnailScrollView:(ThumbnailScrollView *)scrollView cellAtIndex:(NSInteger)index {
    ThumbnailScrollViewCell *cell = [scrollView dequeueReusableCellWithIdentifier:(self.viewMode == 1) ? THUMBNAILVIEW_IDENTIFIER : LISTVIEW_IDENTIFIER];
    if (self.viewMode == 0) //listview mode
    {
        ...
        FileItem *fileObj = [arrayFile objectAtIndex:index];
        ...
        if (fileObj.isFolder) {
            ...
            if ([self hasCalculatedFolderSizeAtPath:fileObj.path]) {
                labelFileSize.text = [Utility displayFileSize:[self folderSizeAtPath:fileObj.path]];
            }
            ...
            [fileObj getThumbnailForPageIndex:-1
                                dispatchQueue:queue
                                  WithHandler:^(UIImage *image, int pageIndex, NSString *pdfPath) {
                                      ...
                                      {
                                          FileItem *imageFileObj = [arrayFile objectAtIndex:index];
            ...
            labelFileSize.text = [Utility displayFileSize:fileObj.fileSize];
        }
    ...
    if (!fileObj.isFolder) {
        ...
        [fileObj getThumbnailForPageIndex:-2
                            dispatchQueue:queue
                              WithHandler:^(UIImage *image, int pageIndex, NSString *pdfPath) {
                                  ...
                                  {
                                      FileItem *imageFileObj = [arrayFile objectAtIndex:imageViewThumbnail.tag];
                                      if ([imageFileObj.path isEqualToString:pdfPath]) {
        ...
        imageViewThumbnail.layer.borderWidth = 0.0;
        labelNumberTitle.text = [NSString stringWithFormat:FSLocalizedString(@"kItems"), fileObj.fileSize];
...
#pragma mark - Calculate folder size at path

- (BOOL)hasCalculatedFolderSizeAtPath:(NSString *)folderPath {
    NSString *key = [[folderPath componentsSeparatedByString:DOCUMENT_PATH] objectAtIndex:1];
    return [[[self class] getFolderSizeDictionary] objectForKey:key] != nil;
}

- (unsigned long long)folderSizeAtPath:(NSString *)folderPath {
    NSString *key = [[folderPath componentsSeparatedByString:DOCUMENT_PATH] objectAtIndex:1];
    if ([[[self class] getFolderSizeDictionary] valueForKey:key]) {
        return [[[[self class] getFolderSizeDictionary] valueForKey:key] unsignedLongLongValue];
    }
    ...
    [[[self class] getFolderSizeDictionary] setValue:[NSNumber numberWithUnsignedLongLong:folderSize] forKey:key];
    return folderSize;
}
...
- (void)thumbnailScrollView:(ThumbnailScrollView *)scrollView blankCellIndexChangedFrom:(NSInteger)fromIndex toIndex:(NSInteger)toIndex {
    ...
    if (fromIndex != -1) {
        FileItem *fileObj = [arrayFile objectAtIndex:fromIndex];
    ...
    if (toIndex != -1) {
        FileItem *fileObj = [arrayFile objectAtIndex:toIndex];
        if (!fileObj.isTouchMoving) {
...
- (void)thumbnailScrollView:(ThumbnailScrollView *)scrollView moreSelectedCellIndex:(NSInteger)cellIndex {
    FileItem *fileObj = [arrayFile objectAtIndex:cellIndex];
...
- (void)thumbnailScrollView:(ThumbnailScrollView *)scrollView didCancelSortCellIndexes:(NSArray *)cellIndexes blankCellIndex:(NSInteger)blankCellIndex {
    if (blankCellIndex != -1) {
        ...
        if (cell != nil) {
            FileItem *fileObj = [arrayFile objectAtIndex:blankCellIndex];
            [UIView animateWithDuration:.3
                                  delay:.0
                                options:UIViewAnimationOptionBeginFromCurrentState
                             animations:^{
    ...
    for (NSNumber *index in cellIndexes) {
        FileItem *fileObj = [arrayFile objectAtIndex:index.intValue];
