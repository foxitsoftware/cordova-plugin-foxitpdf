...
- (void)configureInstructionMessage {
    ...
    CGSize titleSize = [Utility getTextSize:FSLocalizedString(msg) fontSize:20.0f maxSize:CGSizeMake(SCREENWIDTH - 60, 100)];
    ...
    if (lblInstructionMsg == nil) {
        ...
        lblInstructionMsg.text = FSLocalizedString(msg);
...
- (void)initNavigationBar {
    ...
    [buttonLeft setTitle:FSLocalizedString(@"kCancel") forState:UIControlStateNormal];
    ...
    if (self.fileOperatingMode == FileListMode_Select) {
        [self.buttonDone setTitle:FSLocalizedString(@"kSave")];
    } else if (self.fileOperatingMode == FileListMode_Import) {
        [self.buttonDone setTitle:FSLocalizedString(@"kSelectFile")];
        [self.buttonDone setEnabled:NO];
    } else if (self.fileOperatingMode == FileListMode_SaveTo) {
        [self.buttonDone setTitle:FSLocalizedString(@"kSave")];
        self.title = FSLocalizedString(@"kSelectToFolder");
    }
...
- (void)loadFilesWithPath:(NSString *)filePath {
    if (!self.isRootFileDirectory) {
        [self.fileItemsArray addObject:FSLocalizedString(@"kBack")];
    }

    //Avi - added
    [self _loadFilesWithPath:filePath];

    if (self.fileOperatingMode == FileListMode_Select || self.fileOperatingMode == FileListMode_Import) {
