...
@property (nonatomic, weak) FSPDFReader                 *pdfReader;
...
-(void)loadModule
{
    ...
    self.cancelItem = [TbBaseItem createItemWithTitle:NSLocalizedStringFromTable(@"kCancel", @"FoxitLocalizable", nil)];
    ...
    CGSize cancelSize = [Utility getTextSize:NSLocalizedStringFromTable(@"kCancel", @"FoxitLocalizable", nil) fontSize:15.0f maxSize:CGSizeMake(200, 100)];
    ...
    {
        if (weakSelf.extensionsManager.currentAnnot) {
            [weakSelf.extensionsManager setCurrentAnnot:nil];
        }
        
        [weakSelf.extensionsManager showSearchBar:YES];
    };
    [self.pdfReader.topToolbar addItem:searchItem displayPosition:Position_RB];
}
...
- (void)searchTextInPDF:(NSString*)str currentPage:(NSArray*)pages
{
    ...
    __block NSBlockOperation *op = [NSBlockOperation blockOperationWithBlock:^{
        ...
        if (self.arraySearch.count == 0) {
            dispatch_sync(dispatch_get_main_queue(), ^{
                self.foundLable.text = [NSString stringWithFormat:@"%@  %d",NSLocalizedStringFromTable(@"kTotalFound", @"FoxitLocalizable", nil),0];
...
- (void)searchFound:(SearchResult*)result
{
    ...
    self.foundLable.text = [NSString stringWithFormat:@"%@  %d",NSLocalizedStringFromTable(@"kTotalFound", @"FoxitLocalizable", nil),totalCount];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( indexPath.row == 0)  //page summary or show total match count
    {
        ...
        SearchResult *oneResult = [self.arraySearch objectAtIndex:indexPath.section];
        labelLeft.text = [NSString stringWithFormat:@"%@ %d", NSLocalizedStringFromTable(@"kPage", @"FoxitLocalizable", nil), oneResult.index + 1/*page index start from 0*/];
