...
-(void)loadModule
{
    ...
    _pdfReader.signatureItem.onTapClick = ^(TbBaseItem *item) {
...
- (void)initToolBar
{
    ...
    _listItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedStringFromTable(@"kSignListIconTitle", @"FoxitLocalizable", nil) imageNormal:[UIImage imageNamed:@"sign_list"] imageSelected:[UIImage imageNamed:@"sign_list"] imageDisable:[UIImage imageNamed:@"sign_list"] background:nil imageTextRelation:RELATION_BOTTOM];
...
-(void)annotItemClicked
{
    ...
    [_pdfReader changeState:STATE_SIGNATURE];
}

- (void)setToolBarItemHidden:(BOOL)toolBarItemHidden
{
    if (toolBarItemHidden && self.signItem) {
        [_pdfReader.toolSetBar removeItem:self.signItem];
    } else {
        if (!self.signItem) {
        ...
        [_pdfReader.toolSetBar addItem:self.signItem displayPosition:Position_CENTER];
    }
}
