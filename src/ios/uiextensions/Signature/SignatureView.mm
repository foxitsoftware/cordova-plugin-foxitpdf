...
@property (nonatomic,strong) FSPSICallbackImp* callback;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.dib = [FSBitmap create:self.dibWidth height:self.dibHeight format:e_dibArgb buffer:(unsigned char*)dibBuf pitch:0];
...
- (void)initCanvas
{
    self.psiHandler = [FSPSI create:self.dibWidth height:self.dibHeight simulate:YES];
    [self.psiHandler setOpacity:1.0];
    [self.psiHandler setColor:self.color];
    [self.psiHandler setDiameter:self.diameter/0.2];
    if (!self.callback) {
        self.callback = [[FSPSICallbackImp alloc] initWithSigView:self];
    }
    [self.psiHandler setCallback:self.callback];
...
- (void)invalidateRect:(int)left top:(int)top right:(int)right bottom:(int)bottom
{
    ...
    FSRenderer* renderer = [FSRenderer create:self.dib rgbOrder:NO];
