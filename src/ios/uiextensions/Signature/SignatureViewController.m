...
- (void)initSubView
{
    ...
    if (self.currentCertFileName) {
        NSString *title = [NSLocalizedStringFromTable(@"kCurrentSelectCert", @"FoxitLocalizable", nil) stringByAppendingString:self.currentCertFileName];
        [self.createCertBtn setTitle:title forState:UIControlStateNormal];
    }else{
        [self.createCertBtn setTitle:NSLocalizedStringFromTable(@"kSelectAddCert", @"FoxitLocalizable", nil) forState:UIControlStateNormal];
    }
...
- (void)selectCert
{
    ...
    digitalCertSelectCtr.title = NSLocalizedStringFromTable(@"kSelectAddCert", @"FoxitLocalizable", nil);
    [self presentViewController:digitalCertNav animated:YES completion:^{
        
    }];
    typeof(self) __weak weakSelf = self;
    digitalCertSelectCtr.doneOperator = ^(NSString *path, NSString *passwd, NSString *md5){
        NSString *title = [NSLocalizedStringFromTable(@"kCurrentSelectCert", @"FoxitLocalizable", nil) stringByAppendingString:[path lastPathComponent]];
