...
- (instancetype)initWithUIExtensionsManager:(UIExtensionsManager*)extensionsManager
{
    ...
    if (self) {
        ...
        self.isPerformOnce = NO;
        self.isDeleting = NO;
        
        self.docChanging = ^(NSString* newDocPath){
            [extensionsManager.pdfReader openPDFAtPath:newDocPath withPassword:extensionsManager.pdfReader.password];
        };
        self.getDocPath = ^NSString*() {
            return extensionsManager.pdfReader.filePath;
        };
...
- (BOOL)onPageViewTap:(int)pageIndex recognizer:(UITapGestureRecognizer *)recognizer
{
    ...
    if (!_isAdded)
    {
        ...
        MenuItem *signItem = [[MenuItem alloc] initWithTitle:NSLocalizedStringFromTable(@"kSignAction", @"FoxitLocalizable", nil) object:self action:@selector(sign)];
...
{
    FS_PROGRESSSTATE state = [sign startSign:param.signFilePath certPath:param.certFile certPassword:param.certPwd digestAlgorithm:e_digestSHA1 pause:nil clientData:nil];
    while (e_progressToBeContinued == state) {
        state = [sign continueSign];
    }
...
{
    FS_PROGRESSSTATE state = [signature startVerify:nil clientData:nil];
    while (e_progressToBeContinued == state) {
        state = [signature continueVerify];
    }
    *status = [signature getState];
    return e_progressFinished == state;
}
...
- (void)addSignAsImage
{
    ...
    [page startParse:e_parsePageNormal pause:nil isReparse:YES];
...
-(void)signList
{
    ...
    else
    {
        ...
        self.maskView = [[UIControl alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
- (BOOL)onPageViewPan:(int)pageIndex recognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    else if (recognizer.state == UIGestureRecognizerStateChanged)
    {
        ...
        if (!(newRect.origin.x <= 0
              || newRect.origin.x + newRect.size.width >= pageView.frame.size.width
              || newRect.origin.y <= 0
              || newRect.origin.y + newRect.size.height >= pageView.frame.size.height)) {
...
{
    if ([_extensionsManager getCurrentToolHandler] != self || !_isAdded) {
        return;
    }
...
- (void)onDocWillClose:(FSPDFDoc* )document;
{
    ...
    if (self.isShowList) {
        if (DEVICE_iPHONE) {
            [self.signatureListCtr dismissViewControllerAnimated:YES completion:^{
                self.isShowList = NO;
            }];
        }
        else
    ...
    if (self.currentCtr) {
        if ([self.currentCtr isKindOfClass:[SignatureViewController class]]) {
            [(SignatureViewController*)self.currentCtr dismissViewControllerAnimated:NO completion:^{
                self.currentCtr = nil;
            }];
        }
