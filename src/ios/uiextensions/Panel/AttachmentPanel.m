...
@implementation AttachmentPanel {
    FSPDFViewCtrl* __weak _pdfViewControl;
...
- (instancetype)initWithUIExtensionsManager:(UIExtensionsManager*)extensionsManager panelController:(PanelController*)panelController
{
    ...
    if (self) {
        ...
        title.text = NSLocalizedStringFromTable(@"kAttachments", @"FoxitLocalizable", nil);
        ...
        [self.addButton setTitle:NSLocalizedStringFromTable(@"kAdd", @"FoxitLocalizable", nil) forState:UIControlStateNormal];
...
-(void)_addDocumentAttachemnt:(NSString*)attachmentFilePath addUndo:(BOOL)addUndo
{
    ...
    FSPDFNameTree* nameTree = [FSPDFNameTree create:_pdfViewControl.currentDoc type:e_nameTreeEmbeddedFiles];
    if ([nameTree hasName:fileName]) {
        AlertView *alertView = [[AlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"kWarning", @"FoxitLocalizable", nil) message:[NSString stringWithFormat:NSLocalizedStringFromTable(@"kFailedAddAttachmentForExistedName", @"FoxitLocalizable", nil) ,50] buttonClickHandler:^(UIView *alertView, int buttonIndex){} cancelButtonTitle:@"kOK" otherButtonTitles:nil];
        [alertView show];
    } else {
        FSFileSpec* fileSpec = [FSFileSpec create:_pdfViewControl.currentDoc];
        ...
        if ([nameTree add:fileName pdfObj:[fileSpec getDict]]) {
            AttachmentItem* attachmentItem = [AttachmentItem itemWithDocumentAttachment:fileName file:fileSpec PDFPath:_extensionsManager.pdfReader.filePath];
            ...
            AlertView *alertView = [[AlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"kWarning", @"FoxitLocalizable", nil) message:[NSString stringWithFormat:NSLocalizedStringFromTable(@"kFailedAddAttachment", @"FoxitLocalizable", nil) ,50] buttonClickHandler:^(UIView *alertView, int buttonIndex){} cancelButtonTitle:@"kOK" otherButtonTitles:nil];
            [alertView show];
        }
    }
}
...
- (void)addAttachment
{
    ...
    selectDestination.operatingHandler = ^(FileSelectDestinationViewController *controller, NSArray *destinationFolder)
    {
        ...
        if (destinationFolder.count > 0)
        {
            ...
            if (fileSize > 50*1024*1024) { // 50MB
                AlertView *alertView = [[AlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"kWarning", @"FoxitLocalizable", nil) message:[NSString stringWithFormat:NSLocalizedStringFromTable(@"kAttachmentMaxSize", @"FoxitLocalizable", nil) ,50] buttonClickHandler:^(UIView *alertView, int buttonIndex){} cancelButtonTitle:@"kOK" otherButtonTitles:nil];
                [alertView show];
                return;
            }
    ...
    [_pdfViewControl.window.rootViewController presentViewController:selectDestinationNavController animated:YES completion:nil];
}
